...
exports.ContrastValidator = void 0;
...
class ContrastValidator {
    ...
    async validate(componentCode, componentName) {
        try {
            ...
            const errors = violations
                ...
                .map((v) => `${v.type} contrast ${v.actualRatio.toFixed(2)}:1 < ${v.requiredRatio}:1 required (${v.state} state)`);
            const warnings = violations
                .filter((v) => v.actualRatio >= v.requiredRatio && v.actualRatio < v.requiredRatio * 1.2)
                .map((v) => `${v.type} contrast ${v.actualRatio.toFixed(2)}:1 is barely passing (${v.state} state)`);
            return {
     ...
     */
    async testElementContrast(page, state) {
        const violations = [];
        ...
        for (const element of textElements) {
            const ratio = (0, utils_1.calculateContrastRatio)(element.color, element.backgroundColor);
            if (ratio === null)
                continue;
            ...
            if (ratio < requiredRatio) {
                const suggestions = (0, utils_1.suggestAccessibleColors)(element.color, element.backgroundColor, requiredRatio);
                violations.push({
                    type,
        ...
        for (const element of uiElements) {
            const ratio = (0, utils_1.calculateContrastRatio)(element.borderColor, element.parentBackgroundColor);
            if (ratio === null)
                continue;
...
exports.ContrastValidator = ContrastValidator;
//# sourceMappingURL=contrast-validator.js.map