...
class A11yValidator {
     ...
     */
    async validate(componentCode, componentName, variants = ['default']) {
        try {
     ...
     */
    createTestPage(componentCode, componentName, variants) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        for (const violation of results.violations) {
            ...
            for (const node of violation.nodes) {
                ...
                if (violation.impact === 'critical' || violation.impact === 'serious') {
                    ...
                    errors.push(`[${violation.impact.toUpperCase()}] ${violation.id}: ${violation.help} (${node.target.join(', ')})`);
                }
                else {
                    // Moderate and minor violations are warnings
                    warnings.push(`[${violation.impact.toUpperCase()}] ${violation.id}: ${violation.help} (${node.target.join(', ')})`);
                }
            }
        }
...
exports.A11yValidator = A11yValidator;
//# sourceMappingURL=a11y-validator.js.map