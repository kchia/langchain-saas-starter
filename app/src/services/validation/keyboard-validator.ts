...
export class KeyboardValidator {
  ...
  async validate(
    ...
    componentType: 'button' | 'input' | 'modal' | 'dialog' | 'dropdown' | 'tabs' | 'select' | 'link' | 'general' = 'general'
   ...
   */
  private async testActivation(page: import('@playwright/test').Page): Promise<KeyboardIssue[]> {
    const issues: KeyboardIssue[] = [];
    ...
    const enterClicked = await page.evaluate(() => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return (window as any).clickCount > 0;
    });
   ...
   */
  private async testEscapeKey(page: import('@playwright/test').Page): Promise<KeyboardIssue[]> {
    const issues: KeyboardIssue[] = [];
